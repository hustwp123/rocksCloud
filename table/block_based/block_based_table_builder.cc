...
FilterBlockBuilder* CreateFilterBlockBuilder(
    ...
    PartitionedIndexBuilder* const p_index_builder,int output_level) {
  ...
  if (filter_bits_builder == nullptr) {
    ...
    if (table_opt.partition_filters) {
      ...
      if(table_opt.use_pdt&&table_opt.filter_policy->isPdt==true)
      {
        //fprintf(stderr,"OtLexPdtFilterBlockBuilder\n");
...
struct BlockBasedTableBuilder::Rep {
  ...
  Rep(const ImmutableCFOptions& _ioptions, const MutableCFOptions& _moptions,
      ...
      const uint64_t _file_creation_time,int output_level_)
      : ioptions(_ioptions),
        moptions(_moptions),
    ...
    if (skip_filters) {
      ...
      filter_builder.reset(CreateFilterBlockBuilder(
          ...
          use_delta_encoding_for_index_values, p_index_builder_,output_level_));
    }
...
BlockBasedTableBuilder::BlockBasedTableBuilder(
  ...
  rep_ =
      new Rep(ioptions, moptions, sanitized_table_options, internal_comparator,
              ...
              target_file_size, file_creation_time,output_level_);
...
void BlockBasedTableBuilder::WriteFilterBlock(
  ...
  if (ok() && !empty_filter_block) {
    ...
    if (rep_->filter_builder->IsBlockBased()) {
      ...
      {
        if(rep_->table_options.use_pdt&&output_level>=2) {
